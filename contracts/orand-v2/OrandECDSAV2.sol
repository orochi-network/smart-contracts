...
contract OrandECDSAV2 is IOrandECDSAV2 {
  ...
  function _decodeFraudProof(
    bytes memory fraudProof
  ) internal pure returns (OrandECDSAProof memory ecdsaProof, uint256 ecvrfProofDigest) {
    ...
    ecvrfProofDigest = message.readUint256(32);
    ecdsaProof.signer = message.toEthSignedMessageHash().recover(signature);
    return (ecdsaProof, ecvrfProofDigest);
  }

  //=======================[  External View  ]====================

  // Decompose a valid proof
  function decomposeProof(
    bytes memory proof
  ) external pure returns (OrandECDSAProof memory ecdsaProof, uint256 ecvrfProofDigest) {
