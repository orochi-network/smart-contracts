...
// Dependency file: contracts/debug/VRFDebug.sol

// pragma solidity ^0.8.0;

// import '/Users/chiro/GitHub/orosign-contracts/node_modules/hardhat/console.sol';
...
contract VRFDebug {
  ...
  function toHex(uint256 a) public pure returns (string memory) {
    uint256 count = 0;
    uint256 b = a;
    while (b != 0) {
      count++;
  ...
  function fieldHash(bytes memory b) internal view returns (uint256 x_) {
    ...
    console.log('fieldHash(%s) %s', c++, toHex(x_));
    console.logBytes(b);
    while (x_ >= FIELD_SIZE) {
      bytes memory i = abi.encodePacked(x_);
  ...
  function newCandidateSecp256k1Point(bytes memory b) internal view returns (uint256[2] memory p) {
    unchecked {
      ...
      console.log('newCandidateSecp256k1Point(%s): 0x%s 0x%s', ++c, toHex(p[0]), toHex(p[1]));
      if (p[1] % 2 == 1) {
  ...
  function hashToCurve(uint256[2] memory pk, uint256 input) internal view returns (uint256[2] memory rv) {
    ...
    console.log('hashToCurve(%s): 0x%s 0x%s', ++c, toHex(rv[0]), toHex(rv[1]));
    while (!isOnCurve(rv)) {
...
// Root file: contracts/test/OrochiVRFDebug.sol

pragma solidity ^0.8.0;
// import 'contracts/debug/VRFDebug.sol';

contract OrochiVRFDebug is VRFDebug {
